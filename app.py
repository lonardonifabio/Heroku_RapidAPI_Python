...
def index():
    ...
    headings = ("Title","Summary","keywords")
    datafive = db.session.query(News.title,News.link,News.summary,News.keyword).order_by(News.id.desc()).limit(10).all()
...
def search_results(urlsearch):
    ...
    if link != '':
        ...
        return render_template("results.html", search_string = search_string, title = title, summary = summary, image = image, published_date=published_date, author = author, keyword = keyword, cloud = cloud, sentiment_sub = sentiment_sub, sentiment_pol = sentiment_pol, xCC = xCC, xCD = xCD, xDT = xDT, xEX = xEX, xFW = xFW, xIN = xIN, xJJ = xJJ, xJJR = xJJR, xJJS = xJJS, xLS = xLS, xMD = xMD, xNN = xNN, xNNS = xNNS, xNNP = xNNP, xNNPS = xNNPS, xPDT = xPDT, xPOS = xPOS, xPRP = xPRP, xPRP2 = xPRP2, xRB = xRB, xRBR = xRBR, xRBS = xRBS, xRP = xRP, xTO = xTO, xUH = xUH, xVB = xVB, xVBG = xVBG, xVBD = xVBD, xVBN = xVBN, xVBP = xVBP, xVBZ= xVBZ, xWDT = xWDT, xWP = xWP, xWRB = xWRB)
